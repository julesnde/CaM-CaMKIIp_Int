...
chi_min = 14.0 # min for PMF
chi_max = 31.0 # max for PMF
#nbins = 48 # number of bins for 1D PMF
nbins = 69
...
for k in range(K):
    ...
    K_k[k] = float(tokens[1])    # spring constant (read in kJ/mol/AA**2).
    if len(tokens) > 2:
...
for k in range(K):
    ...
    filename = 'data/CaM_CaMKIIp_Stab_%d.txt' % (k+1)
    ...
    for line in lines:
        if line[0] != '#' and line[0] != '@':
            tokens = line.split()
            chi = float(tokens[0]) # torsion angle
            chi_kn[k,n] = chi
            n += 1
    N_k[k] = n
